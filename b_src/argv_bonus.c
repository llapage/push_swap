...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/13 14:59:20 by llapage           #+#    #+#             */
/*   Updated: 2023/06/13 15:04:05 by llapage          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_duplicates(char **argv)
{
	...
	while (argv[i])
	{
		n = ft_atoi_ps_2(argv[i]);
		if (ft_strlen(argv[i]) > 11 || n > 2147483647 || n < -2147483648)
		{
			free(tab);
			return (-1);
		}
		tab[i] = ft_atoi_ps_2(argv[i]);
		i++;
	}
	if (check_tab_2(tab, matrix_len(argv)) == -1)
		return (-1);
	return (0);
}
